...
class ROMTRACK_ALPHA_SEG(object):
    def __init__(self, tracker_name='ROMTrack', para_name='baseline_stage2',
                 refine_model_name='ARcm_coco_seg', threshold=0.15, tracker_params=None):
        self.THRES = threshold
        '''create tracker'''
        '''ROMTrack'''
        tracker_info = Tracker(tracker_name, para_name, "vot20", None, tracker_params=tracker_params)
        params = tracker_info.get_parameters(tracker_params)
        params.visualization = False
        params.debug = False
        params.visdom_info = {'use_visdom': False, 'server': '127.0.0.1', 'port': 8097}
        self.romtrack = tracker_info.create_tracker(params)
        '''Alpha-Refine'''
    ...
    def initialize(self, img_RGB, mask):
        ...
        '''Initialize ROMTrack for specific video'''
        init_info = {'init_bbox': list(gt_bbox_np)}
        _ = self.romtrack.initialize(img_RGB, init_info)
    ...
    def track(self, img_RGB):
        '''TRACK'''
        '''base tracker'''
        outputs = self.romtrack.track(img_RGB)
...
def run_vot_exp(tracker_name, para_name, refine_model_name, threshold, VIS=False, tracker_params=None):
    ...
    tracker = ROMTRACK_ALPHA_SEG(tracker_name=tracker_name, para_name=para_name,
                              refine_model_name=refine_model_name, threshold=threshold, tracker_params=tracker_params)
    handle = vot.VOT("mask")
