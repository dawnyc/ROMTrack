...
# from lib.test.pytracking.analysis.extract_results_new import extract_results, extract_results_prec_rec_f1
...
def plot_radar_draw_save(scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    all_score_per_tracker = torch.Tensor([scores['ALL'][i] for i in range(len(trackers))])

    x_tick_labels = []
    for att in attributes:
        x_tick_labels.append('\n'.join(att.split(' ')))
        min_att_score = min(scores[att])
        max_att_score = max(scores[att])
        x_tick_labels[-1] = x_tick_labels[-1] + "\n({:.1f}~{:.1f})\n".format(min_att_score, max_att_score)
    ...
    index_sort = all_score_per_tracker.argsort(descending=False)

    plotted_lines = []
    legend_text = []

    # pre-process with scores: normalize to 1.0
    for att in attributes:
        criteria = scores[att][index_sort[-1]]
        for i in range(0, len(index_sort)):
            if i == len(index_sort) - 1:
                continue
            scores[att][index_sort[i]] = scores[att][index_sort[i]] / criteria
        scores[att][index_sort[-1]] = 1.0
    ...
    for id, id_sort in enumerate(index_sort):
        ...
        if len(disp_name) > 3 and disp_name[:3] == r'\bf':
            ...
            legend_text.append(r'$\mathbf{{{}}}$ $\mathbf{{[{:.1f}]}}$'.format(disp_name[3:], all_score_per_tracker[id_sort]))
        else:
            # legend_text.append('{} [{:.1f}]'.format(disp_name, avg_score_per_tracker[id_sort]))
            legend_text.append('{} [{:.1f}]'.format(disp_name, all_score_per_tracker[id_sort]))


    ax.legend(plotted_lines[::-1], legend_text[::-1], loc=legend_loc, fancybox=False, edgecolor='white',
              fontsize=font_size_legend, framealpha=1.0, handlelength=handlelength, handletextpad=0.3,
              bbox_to_anchor=bbox_to_anchor, ncol=min(len(trackers), 6))
