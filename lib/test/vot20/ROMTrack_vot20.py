...
'''ROMTrack_vot20 class'''


class ROMTrack_vot20(object):
    def __init__(self, tracker_name='ROMTrack', para_name='baseline_stage2', tracker_params=None):
        # create tracker
...
def run_vot_exp(tracker_name, para_name, vis=False, tracker_params=None):

    torch.set_num_threads(1)
    save_root = os.path.join('<SAVE_DIR>', para_name)
    if vis and (not os.path.exists(save_root)):
        os.mkdir(save_root)
    tracker = ROMTrack_vot20(tracker_name=tracker_name, para_name=para_name, tracker_params=tracker_params)
    handle = vot.VOT("mask")
