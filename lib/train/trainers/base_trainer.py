...
class BaseTrainer:
        ...
        for i in range(num_tries):
            try:
                if load_latest:
                    ...
                    self.load_checkpoint("models-finetune", ["net"])
                    # self.load_checkpoint("models-finetune", ["net", "optimizer", "epoch", "stats"])
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if self.lr_scheduler is not None:
                        ...
                        # cosine decay
                        lr1 = 1e-6 + (1e-4 - 1e-6) * 0.5 * (1. + math.cos(math.pi * epoch / max_epochs))
                        lr2 = 1e-6 + (1e-5 - 1e-6) * 0.5 * (1. + math.cos(math.pi * epoch / max_epochs))
                        self.optimizer.param_groups[0]["lr"] = lr1
                        self.optimizer.param_groups[1]["lr"] = lr2
    ...
    def save_checkpoint(self):
        ...
        # torch.save(net.state_dict(), tmp_file_path)
