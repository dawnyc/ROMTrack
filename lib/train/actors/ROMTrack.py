...
class ROMTrackActor(BaseActor):
    ...
    def forward_pass(self, data, stage, run_score_head):
        search_bboxes = box_xywh_to_xyxy(data['search_anno'][-1].clone())
    ...
    def compute_losses(self, pred_dict, gt_bbox, return_status=True, labels=None):
        # gt gaussian map
        gt_gaussian_maps = generate_heatmap(gt_bbox, self.settings.output_sz['search'], self.settings.patch_size)  # example: output_sz=256, patch_size=16

        gt_gaussian_maps = gt_gaussian_maps[-1].unsqueeze(1)
        gt_bbox = gt_bbox[-1]

        return self.compute_losses_one_search(pred_dict, gt_bbox, gt_gaussian_maps, return_status, labels)

    def compute_losses_one_search(self, pred_dict, gt_bbox, gt_gaussian_maps, return_status=True, labels=None):
        # Get boxes
        ...
        # compute giou/ciou/siou and iou
        try:
