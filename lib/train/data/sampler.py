...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def _sample_visible_ids_for_search(self, visible, num_ids=1, min_id=None, max_id=None,
                            allow_invisible=False, force_invisible=False):
        """ Samples num_ids frames between min_id and max_id for which target is visible
        ...
        if num_ids == 2:
            r = random.choices(valid_ids, k=1)
            r_idx = valid_ids.index(r[0])
            if r_idx == 0:
                return r * 2
            else:
                return [valid_ids[r_idx-1]] + r
        else:
            return random.choices(valid_ids, k=num_ids)
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    while search_frame_ids is None:
                        ...
                        search_frame_ids = self._sample_visible_ids_for_search(visible,
        ...
        while not valid:
            ...
            try:
                ...
                # negative samples
                    ...
                    # else:
                    #     search_frames, search_anno, meta_obj_test = self.get_one_search()
                    # TODO modified(two frames)
                    if is_video_dataset:
                        search_frame_ids = self._sample_visible_ids_for_search(visible, num_ids=2, force_invisible=True)
                        if search_frame_ids is None:
                            search_frames, search_anno, meta_obj_test = self.get_two_search()
                        else:
                            search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids,
                                                                                           seq_info_dict)
                            search_anno["bbox"] = [self.get_center_box(H, W)]
    ...
    def get_two_search(self):
        ...
        # sample two frames
        if is_video_dataset:
            search_frame_ids = self._sample_visible_ids_for_search(visible, num_ids=2, allow_invisible=True)
        else:
            search_frame_ids = [1] * 2
        # get the image, bounding box and other info
        search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)
    ...
    def get_frame_ids_trident(self, visible):
        ...
        while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0:
            ...
            template_frame_id1 = self._sample_visible_ids(visible, num_ids=3)  # TODO modified(num_ids 1 -> 2)
            # search_frame_ids = self._sample_visible_ids(visible, num_ids=1)  # the search region id
            search_frame_ids = self._sample_visible_ids_for_search(visible, num_ids=2)  # TODO modified(..._for_search added, num_ids 1 -> 2)
            # get the dynamic template id
            for max_gap in [self.max_gap]:  # TODO modified([] added)
