...
class Corner_Predictor(nn.Module):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False):
        ...
        with torch.no_grad():
            ...
            # generate mesh-grid
            # [0,16,...,288,304,0,16,...,288,304,...,0,16,...,288,304,0,16,...,288,304]
            self.coord_x = self.indice.repeat((self.feat_sz, 1)) \
                .view((self.feat_sz * self.feat_sz,)).float().cuda()
            # [0,0,...,0,0,16,16,...,16,16,...,288,288,...,288,288,304,304,...,304,304]
...
def build_box_head(cfg, stride=16):
    if cfg.MODEL.HEAD_TYPE == "MLP":
        ...
        print("CORNER head channel: %d" % channel)
        ...
        channel = getattr(cfg.MODEL, "HEAD_CHANNEL", 256)
        freeze_bn = getattr(cfg.MODEL, "HEAD_FREEZE_BN", False)
        print("CENTER head channel: %d" % channel)
        if cfg.MODEL.HEAD_TYPE == "CENTER":
            center_head = CenterPredictor(inplanes=cfg.MODEL.HIDDEN_DIM, channel=channel,
                                          feat_sz=feat_sz, stride=stride, freeze_bn=freeze_bn)
        else:
            raise ValueError()
        return center_head
